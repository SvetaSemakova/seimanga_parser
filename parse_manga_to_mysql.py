"""
Скрипт для парсинга каталогов манги с сайтов seimanga и zazaza
и сохранения данных в базу MySQL.

ВАЖНО:
- Перед использованием проверьте robots.txt сайта и условия использования —
  массовый парсинг может быть запрещён правилами сайта.
- Добавьте задержку между запросами (time.sleep), чтобы не перегружать сервер
  и не попасть под бан по IP.
- CSS-селекторы ниже — ШАБЛОН. Их нужно уточнить, открыв страницу сайта
  в браузере (F12 -> Elements) и посмотрев реальные классы/теги.
"""
